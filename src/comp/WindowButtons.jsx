...
const IS_TAURI = !!(window && window.__TAURI_INTERNALS__)
const tauriGetWindow = IS_TAURI ? getCurrentWindow : (() => ({
    minimize: () => { },
    toggleMaximize: () => { },
    close: () => { },
    onResized: () => { },
...
const appWindow = tauriGetWindow();
...
export const MinimizeIcon = ({ color = 'currentColor', size = 18 }) => {
    if (!IS_TAURI) return <div></div>
    return <div className="minimize-icon" onClick={minimizeWindow}>
...
export const CloseIcon = ({ color = 'currentColor', size = 18 }) => {
    if (!IS_TAURI) return <div></div>
    return <div className="close-icon" onClick={closeWindow}>
        <svg width={size} height={size}
            viewBox="0 0 24 24" fill="none"
            stroke={color} stroke-width="2"
